...
function RegistrationsModal({title,subtitle, deleteStatus = true, onDelete, onClose, data = [] }) {
  ...
  return (
    ...
    <div
      className="absolute inset-0 z-10 flex min-w-0 flex-col rounded-sm bg-[#090d16]/95 backdrop-blur-sm"
      onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}
    >
      {/* Header */}
      <div className="flex shrink-0 items-start justify-between gap-3 border-b border-white/10 px-3 py-2.5">
        <div className="min-w-0">
          <p className="break-words font-mono text-[10px] uppercase tracking-[2px] text-cyan-300">
            {title}
          </p>
          <h2 className="break-words font-mono text-sm font-bold uppercase tracking-wider text-white">
            {subtitle}
        ...
        <button
          type="button"
          onClick={onClose}
          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-sm border border-red-400/50 font-mono text-sm text-red-400 transition hover:bg-red-400 hover:text-black focus-visible:ring-2 focus-visible:ring-red-400"
          aria-label="Close"
        >
          ×
        </button>
          ...
          <table className="min-w-[520px] w-full border-collapse font-mono text-xs">
            ...
            <tbody>
              {registrations.map((user, index) => (
                ...
                >
                  <td className="max-w-32 break-all px-2 py-2 text-cyan-200">{user.username}</td>
                  <td className="max-w-40 break-words px-2 py-2">{user.fullname}</td>
                  <td className="px-2 py-2 text-gray-400 hidden sm:table-cell">{user.number}</td>
                  <td className="hidden max-w-[160px] truncate px-2 py-2 text-gray-400 sm:table-cell">{user.email}</td>
                  ...
                  {deleteStatus && (
                    <td className="px-2 py-2 text-right">
                      <button
                        ...
                        className="ml-auto flex h-9 w-9 items-center justify-center rounded-sm border border-red-400/40 text-red-400 transition hover:bg-red-400 hover:text-black focus-visible:ring-2 focus-visible:ring-red-400"
